...
Структура:Приложение использует MaterialApp для настройки и Scaffold для базовой структуры экрана.
...
Методы и виджеты:
MaterialApp:Корневой виджет приложения, задает настройки, такие как заголовок (title), тема (theme) и начальный экран (home).
...
Scaffold:Предоставляет базовую структуру экрана Material Design.

body: содержит основной контент (здесь ListView).

ListView:Виджет для создания прокручиваемого списка.

children: список дочерних виджетов, которые отображаются в порядке добавления (здесь ListTile).

ListTile:Элемент списка, предназначенный для отображения строки с иконкой и текстом.

leading: виджет слева от текста (здесь иконка из Icons).

title: основной текст элемента (здесь Text с номерами).

Icon:Отображает иконку из библиотеки Material Design.

Принимает значение из Icons (например, Icons.gif, Icons.book).

Text:Отображает текст в заголовках элементов списка.
