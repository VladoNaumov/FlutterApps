...
Этот код создаёт Flutter-приложение, которое отображает карточку (Card) в центре экрана.
Карточка содержит три текстовых элемента ("Child1", "Child2", "Child3"),
расположенных вертикально и разделённых линиями (Divider).
Приложение использует StatelessWidget, так как интерфейс статичен и не требует управления состоянием.

Разбор ключевых компонентов


Класс MyApp и StatelessWidget:
MyApp наследуется от StatelessWidget, так как интерфейс не требует изменения состояния
(например, обработки нажатий или динамического контента).
