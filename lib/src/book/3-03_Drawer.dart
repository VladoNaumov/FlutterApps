...
/*
Этот код создаёт простое Flutter-приложение с боковым меню (Drawer) и основным экраном,
на котором три текстовых элемента расположены горизонтально.
Боковое меню содержит три пункта, разделённых линиями (Divider). П
риложение использует StatelessWidget, так как не требует управления состоянием.

Разбор ключевых компонентов

Класс MyApp и StatelessWidget:
MyApp наследуется от StatelessWidget, так как интерфейс статичен и не требует изменения состояния
(например, переключения страниц или обработки нажатий).
