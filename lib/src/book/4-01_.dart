...
разделенных пустым пространством (Spacer).
Каждая секция демонстрирует разные возможности Flutter для работы с виджетами,
...
Scaffold:
Предоставляет структуру экрана (например, body для содержимого).

Column:
Располагает дочерние виджеты вертикально.

Spacer:
Занимает доступное пространство, равномерно распределяя его между виджетами.

Center:
Центрирует дочерний виджет.

Opacity:
Управляет прозрачностью (opacity: 0.25 = 25% видимость).

Theme:
Локально изменяет тему через Theme.of(context).copyWith.

Используется для задания colorScheme.secondary для конкретного виджета.

Container:
Задает стиль (цвет, размеры, отступы) и компоновку.

Используется для фона и контейнеров с текстом.

DecoratedBox:
Добавляет декоративный фон, например, градиент через BoxDecoration.

LinearGradient:
Создает линейный градиент с указанными цветами и направлениями.

Transform:
Применяет геометрические трансформации.
...
Text:
Отображает текст с настраиваемым стилем (TextStyle).
